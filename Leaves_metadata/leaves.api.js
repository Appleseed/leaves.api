class LeavesStaticClass {
    ...
    static leavesFirstPara(url, res){
                var request = require('request');
                ...
                request(url, function (error, response, html) {
                    ...
                    if (!error && response.statusCode == 200) {
                       ...
                       $('article.markdown-body.entry-content').each(function(i, element){
                          var a = $(this).children().next().next();
                          var firstPara = a.eq(0).text();
                          if (firstPara == "") {
                             firstPara = a.eq(1).text();
                          }
                          console.log(firstPara);
                          return res.status(200).send(firstPara); 
                       })
		    }
